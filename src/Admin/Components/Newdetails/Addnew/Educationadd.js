...
const Educationadd = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('https://tulirmatrimony.com/controlapi/addeducation.php', {
                method: 'POST',
                headers: {
  ...
  return (
   ...
   <Aside />
                ...
                <div class="card-body">
                    <form id="addEditForm" ref={formRef} name="addEditForm" onSubmit={handleSubmit} method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="_token" value="xsRbVQIcWzUtaB16B9EQu0T5IiltWdJYP6iUnE2Q"/>                        <div class="mb-3 text-start">
                                        ...
                                        <input type="text" required="" class="form-control required" id="Education_name" name="Education_name"  value={education} onChange={(e) => setEducation(e.target.value)} placeholder="Education " />
    ...
    <Link to="#" className="btn btn-secondary m-4" onClick={handleGoBack}> Go Back EducationList </Link>
</div>
