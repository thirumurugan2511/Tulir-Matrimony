...
const Education = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch('https://tulirmatrimony.com/controlapi/educationlist.php');
  ...
  const handleDelete = async (id) => {
    try {
        await fetch(`https://tulirmatrimony.com/controlapi/deleteeducation.php`, {
            method: 'DELETE',
            headers: {
...
<div class="col-md-2">
            <a href="/Educationadd" class="fw-bold mb-4 btn btn-info"><IoMdAdd class="bx bx-plus-circle" /> Add New</a>
        </div>
...
{/* <div class="col-md-4">
   ...
   <div class="card text-start">
       <h5 class="card-header">Manage Education</h5>
   ...
   {data.map(item => (
            <tr key={item.id}>
                <td>{item.name}</td>       
                <td><Link to={`/Educationedit/id=${item.id}`} className='text-ed'><MdModeEdit class="bx bxs-edit"/> Edit</Link> /
