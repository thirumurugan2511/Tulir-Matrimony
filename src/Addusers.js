...
const Addusers = () => {
  ...
  const handleChange = (e) => {
    ...
    if (["image", "image1", "id_image", "id_image1", "rasiimage"].includes(name)) {
      ...
      // Calculate age if date of birth is selected
      if (name === 'dob') {
  ...
  const generateStateOptions = () => {
    ...
    return statesInIndia.map((state) => ({
      value: state.name,
      label: state.name,
    }));
  };
    ...
    try {
      ...
      switch (fieldName) {
        ...
        case "country":
          ...
          setOptions((prevOptions) => ({
            ...prevOptions,
            [fieldName + "Options"]: [{ value: "india", label: "India" }],
          }));
          break;
      ...
      if (endpoint !== "") {
        ...
        if (response.ok) {
          ...
          switch (fieldName) {
            case "caste":
              options = responseData.body.map((item) => ({
                value: item.caste_name,
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const combinedData = Object.values(formData).reduce(
        (acc, sectionData) => {
      ...
      const imageData = {
        ...
        id_image: combinedData.id_image
          ? combinedData.id_image.split(",")[1]
          : null,
        id_image1: combinedData.id_image1
          ? combinedData.id_image1.split(",")[1]
          : null,
        rasiimage: combinedData.rasiimage
          ? combinedData.rasiimage.split(",")[1]
          : null,
      };
          ...
          >
            ...
            >
              <ol class="breadcrumb breadcrumb-style2 mb-0">
                ...
                <li class="breadcrumb-item" style={{ padding: "2px 10px" }}>
                  {" "}
                  Manage Member{" "}
                </li>
              </ol>
                ...
                <div className="edit_inputMain-sltr row">
                  ...
                  {Object.keys(formData[currentSection]).map((fieldName) =>
                    [
                      ...
                      <div
                        key={fieldName}
                        className="col-lg-4 col-md-6 mb-4 text-start"
                      >
                        ...
                        >
                          <option value="">
                            Select your{" "}
                            {fieldName === "partner_from_age"
                              ? "Partner from age"
                              : fieldName === "partner_to_age"
                              ? "Partner to age"
                              : fieldName === "partner_religion"
                              ? "Partner religion"
                              : fieldName === "partner_country"
                              ? "Partner country"
                              : fieldName === "partner_state"
                              ? "Partner state"
                              : fieldName === "partner_matrial_status"
                              ? "Partner marital status"
                              : fieldName === "partner_education"
                              ? "Partner education"
                              : fieldName === "partner_occupation"
                              ? "Partner occupation"
                              : fieldName === "partner_mother_tongue"
                              ? "Partner mothertongue"
                              : fieldName === "partner_manglik"
                              ? "Partner manglik"
                              : fieldName === "partner_salary"
                              ? "Partner salary"
                              : fieldName === "partner_child_gender"
                              ? "Partner child gender"
                              : `${
                                  fieldName
                                    .replace(/_/g, " ")
                                    .charAt(0)
                      ...
                      <div
                        key={fieldName}
                        className="col-lg-4 col-md-4 mb-4 text-start"
                      >
                        <label htmlFor={fieldName} style={{ color: "black" }}>
                          {labelTranslations[fieldName]}
                          {[
                        ...
                        {[
                          ...
                          <input
                            ...
                            name={fieldName}
                            placeholder={
                              fieldName === "partner_height"
                                ? "Partner Height"
                                : fieldName === "partner_weight"
                                ? "Partner Weight"
                                : fieldName === "partner_caste"
                                ? "Partner Caste"
                                : fieldName === "partner_child_count"
                                ? "Partner Child Count"
                                : fieldName === "partner_child_age"
                                ? "Partner Child Age"
                                : `${
                                    fieldName
                                      .replace(/_/g, " ")
